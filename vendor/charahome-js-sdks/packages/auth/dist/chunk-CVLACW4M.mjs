...
var CHARAHOMELinkingClient = class {
   ...
   * 4. Ensure UserData exists (if ensureUser is provided in apiClient)
   */
  async ensureAuthenticated() {
    ...
    if (this.apiClient.ensureUser) {
      const token = await charahomeUser.getIdToken();
      await this.apiClient.ensureUser(token);
    }
  ...
  async mergeWithExistingAccount(anonUid, existingUid, derivedUid, email, password) {
    ...
    const result = await this.apiClient.mergeAccounts(body, token);
