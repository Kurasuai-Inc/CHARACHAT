...
__export(index_exports, {
  CHARAHOMEAuthFlowError: () => CHARAHOMEAuthFlowError,
  CHARAHOMEAuthProvider: () => CHARAHOMEAuthProvider,
  CHARAHOMELinkingClient: () => CHARAHOMELinkingClient,
  createAuthApiClient: () => createAuthApiClient,
  createBrowserAuth: () => createBrowserAuth,
  createDerivedAppAuthConfig: () => createDerivedAppAuthConfig,
  createLinkingClient: () => createLinkingClient,
  getAuthInstances: () => getAuthInstances,
  getOrInitializeApp: () => getOrInitializeApp,
  getProjectId: () => getProjectId,
  initializeDualAuth: () => initializeDualAuth,
  resolveAuthApiClient: () => resolveAuthApiClient,
  useAuth: () => useAuth
});
...
// src/config.ts
var import_client = require("@kurasuai-inc/charahome-api/client");
function createDerivedAppAuthConfig(input) {
  const environment = input.environment ?? "staging";
  const environmentConfig = (0, import_client.getEnvironmentConfig)(environment);
  return {
    derivedAppConfig: input.derivedAppConfig,
    derivedAppName: input.derivedAppName,
    charahomeConfig: input.charahomeConfig ?? (0, import_client.getFirebaseConfig)(environment),
    appId: input.appId,
